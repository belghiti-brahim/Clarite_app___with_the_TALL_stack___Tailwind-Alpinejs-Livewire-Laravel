       ...
       <div class="shadow sm:rounded-md sm:overflow-hidden">
           <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
               ...
               @if (!is_null($projects))
                   <div class="col-span-6 sm:col-span-3">
                       <label for="project"
                           class="block text-sm font-medium text-gray-700">{{ __('Projects') }}</label>
           ...
           <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
               <button {{ empty($actionDescription) ? 'disabled' : '' }} type="submit" wire:click="submit"
                   class="btn">{{ __('save') }}</button>
           </div>
       </div>
   </div>
